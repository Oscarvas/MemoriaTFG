...
Si quieres dar variedad a tu historia esta debe contar con una gran diversidad de personajes, cada uno con sus virtudes y limitaciones, y a cuantos más personajes haya más posibilidades se tendrá de crear diferentes historias, ya que cada uno podría actuar de manera distinta en cada ejecución. 

Con este objetivo en mente se definió un nuevo diseño para los personajes. Sin embargo las posibles historias que se pueden generar comparten siempre una misma estructura. 
El hilo principal desencadenante de la historia es que siempre aparecerá un antagonista que logra secuestrar a la princesa por lo que el rey pedirá ayuda a todos los aspirantes a héroes disponibles y estos acudirán a su llamada para ir a por la princesa, a partir de este momento lo que sucede en la historia es aleatorio.
Para poder diseñar correctamente los diversos personajes que participan en la historia lo primero que se debe hacer es recoger unos tipos para los personajes que definiesen cual sería su importancia y la manera de participar en la historia. De esta manera dividimos a los personajes en tres bloques específicos: \texttt{Protagonistas}, \texttt{Antagonistas} y \texttt{Personajes No Jugadores} o \texttt{PNJs}.

Así los \texttt{Protagonistas} serán los encargados de llevar el peso de la historia, representando aquellos personajes con los que el usuario se puede identificar y asumirían el rol de los ``buenos'' dentro de la narración. Por su parte los \texttt{Antagonistas} actuaran en contra de los intereses de los protagonistas, ocasionando problemas que estos deben resolver durante el transcurso de la historia y asumiendo el papel de ``malos'' en la misma. Por último, los \texttt{PNJs} representarán los personajes secundarios que intervendrían en una narración, modificando la manera en la que esta discurre pero sin tener un papel protagonista en la historia, sino mas bien un papel de apoyo u oposición con respecto a los protagonistas y puediendo llegar a ser claves en la clonclusiones de los conflictos.

También es importante indicar que todos los personajes se planifican en algún momento de su ejecución. Para entender realmente como se planifican y el papel que juega el planificador en su manera de ejecutarse, es recomendable comprender el capitulo \ref{cap:Arquitectura:Planificador}.
...
Para que la historia fuese rica se necesitará de no solo uno sino varios personajes principales con los que disponer en el transcurso natural de la historia, pues cuantos mas personajes hubiera, mejores historias se podran crear. Lo natural en estos casos es que los protagonistas no sean meras copias unos de otros, así que dentro de los propios protagonistas se establece una nueva división de tipos.

\subsection{Diseño de los Protagonistas}
En la búsqueda de unos protagonistas para nuestras historias decidimos partir de aquellos ya planteados, como se ve en \ref{cap:PrimerosPasos}, pero para que estos personajes fuesen originales y pudiesen cumplir papeles diversos en la historia fue necesaria la inclusión del concepto de clase dentro de los protagonistas. Estas clases ayudaran a parametrizar los pasos que seguirá cada clase de personajes dentro de la historia y así darle a cada clase ciertas funcionalidades específicas. 
Por tanto es importante remarcar que los protagonistas serán los únicos que podrán obtener y consumir recursos, tal y como se explica en el capítulo de objetos \ref{cap:Objetos}.
Las distintas clases a las que un protagonista puede pertenecer son:

\subsubsection{La Princesa}
La Princesa cumple un rol muy importante dentro de la historia. Ella siempre era raptada por un antagonista y a partir de ese momento desencadenaba la acción de los distintos protagonistas repartidos por el mundo, y las diversas pruebas por las que debían pasar hasta que por fin fuese salvada.
...
Como se explicó en el capítulo de objetivos, unos de estos ha sido conseguir que la aplicación fuese altamente personalizable haciéndola lo menos restrictiva posible.
Para ello, se decide que los Protagonistas pudiesen ser configurados mediante un archivo XML. Este archivo brinda la oportunidad de crear clases menos específicas y que se adaptasen a mas entornos, en definitiva que fuesen mas genéricas. Con este motivo en mente se cambió el nombre de las clases explicadas anteriormente pudiéndose ver el resultado en la figura \ref{fig:TablaClasesProtagonistas}.
...
Esta forma permitirá al usuario tener mayor flexibilidad a la hora de concretar los parámetros y el ámbito de la historia, de manera que puede elegir que uno de los héroes en su historia sea el caballero Don Quijote embarcado en el peligroso rescate de la princesa Dulcinea o que el samurai Kenshin deba rescatar a Mulán, la hija del emperador. 

%-------------------------------------------------------------------


\subsection{Implementación de los Protagonistas}
Para poder desarrollar de manera eficiente los Protagonistas hubo que realizar numerosas tareas en la aplicación, ya que en parte partíamos de cero, tal y como se explicó en el capitulo \ref{cap:PrimerosPasos}. Esto nos llevo a necesitar de distintas clases para poder cumplir los objetivos que a continuación se detallan. Cada acción que aquí se comenta, como que la princesa pide ser rescatada, aparecerá en la historia(ver con detalle en \ref{cap:Simulaciones}).

%-------------------------------------------------------------------
\subsubsection{Carga y Configuración de los Protagonistas}
Al tener unos Protagonistas tan diversos se necesita de ``algo'' capaz de interpretar correctamente los datos de estos. Así nació el concepto del Sistema de Carga de Datos (SCD), cuyo diseño se puede ver en profundidad en el capitulo \ref{cap:Arquitectura:Loaders}. Este SCD es capaz de cargar e interpretar los datos proporcionados a través del fichero ``Personajes.XML'', a partir del cual se cogen las características concernientes a cada Protagonista en específico y proporcionaba al agente mundo esos datos, recorriendo en el archivo todos los nodos con la palabra clave \texttt{<personaje>}.
