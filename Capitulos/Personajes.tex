...
Antes de empezar a considerar lo que cada personaje en específico es capaz de hacer y que función tiene pensada para el desarrollo de la historia, es importante investigar la clase personaje.

La \texttt{clase personaje} representa un papel fundamental en la implementación de la aplicación. En concreto, la clase personaje nos proporciona una superclase a partir de la cual el resto de clases se desarrollan. Para ello cuenta con diferentes métodos y atributos comunes a los distintos tipos de personajes. 

Así, gracias a esta clase, todos los agentes que representan personajes comparten ciertos métodos como por ejemplo, el método \texttt{cargarMundo}, que les permite cargar la información relativa al mundo que han de conocer y localizarse en el mapa y moverse a través de él, gracias también a la ayuda proporcionada por los métodos \texttt{localizarInicial} y \texttt{localizarPersonaje}. Así mismo, gracias al método \texttt{planificar} todos los personajes son capaces de planificarse o usando el método \texttt{mandarCrearArchivo} son capaces de crear sus archivos \texttt{.PDDL}.
Aparte, también comparten ciertos atributos comunes a todos los personajes, como son la vida del personaje, su sexo, su localización, su casa... junto a los diferentes métodos \textit{getters} y \textit{setters} para poder usarlos. 

Por lo tanto y gracias a esta clase, se puede unificar el código de una manera más sencilla, permitiendo evitar la redundancia de código y proponiendo una superclase común sobre un elemento que así lo requiere, provocando que la aplicación sea mas intuitiva y fácil de remodelar para cualquier persona que quiera proseguir con nuestra investigación.
...
Sin embargo, los distintos personajes no comparten solo atributos y métodos, también tienen en común ciertas acciones, \texttt{Behaviours} de Jade, que representan aquellas actividades que todos los personajes pueden hacer. Hay que resaltar que los PNJ's, al tratarse de personajes mas planos, aunque puedan realizar estas acciones en el transcurso natural de la historia nunca lo harán.
Estas acciones no estan incluidas dentro de la \texttt{clase personaje}, pero sin embargo es desde esta clase desde donde se invoca su funcionamiento, normalmente a través del metodo \texttt{planificar}.
...
Por ejemplo, está el comportamiento \texttt{Batalla}. Este comportamiento expone como los personajes resolverán las disputas que tengan entre ellos. 
Para el funcionamiento de \texttt{Batalla}, los personajes restan la suma de su vida y su atributo principal a la vida de su contrincante, mientras que los antagonistas restan directamente su vida. Aquel que consigue que la vida de su oponente sea menor que cero sería el vencedor. En caso de que ambos lo consigan, ambos mueren.
...
El comportamiento \texttt{AyudaArcana()} es un comportamiento complejo, en el que participan varios agentes y se comunican intercambiando mensajes. 

En resumen, cuando un antagonista guardian se encuentra con un aspirante a héroe, este pedirá ayuda a algún mago disponible y el mago acudira entablando batalla con el guardián
%-------------------------------------------------------------------
...
El comportamiento \texttt{Maldad} es un comportamiento cíclico, un \texttt{CyclicBehaviour}, es decir, que ocrure cada cierto tiempo. En él, el \texttt{Antagonista Asesino} elegirá a aquel \texttt{Allegado a la Víctima} cuya \texttt{Víctima} haya sido capturada, a través de un mensaje ACL del tipo \texttt{Inform}, y por tanto sea derrocable. Tras esto se planificará, ejecutando la acción \texttt{Tortura}, indicando al allegado a traves de el mensaje ACL del tipo \texttt{Inform Agoniza} que su vida va disminuyendo poco a poco, concretamente cada 30 segundos.
