...
Al iniciarse el agente se cargaban sus datos específicos de la manera general,con la particularidad de que también se cargaba el salario, cualidad que usaba el agente para contratar los servicios de los héroes que quería que intentasen salvar a la víctima. 
%quitable la siguiente frase
También recopila los datos referentes al numero de víctimas a las que esta llegado y se localiza, tras lo que registra su tipo como ``Rey'' y comienza su ejecución, registrando un FSMBehaviour.
A partir de aquí el funcionamiento del rey es ciertamente delicado. Su ejecución se asemeja a la de una máquina de estados,%esto es verdad?
 donde cada comportamiento se ejecutará una única vez y da paso al siguiente. Para poder ejecutarse correctamente ha de registrar los estados, añadiendo su comportamiento correspondiente al estado junto al nombre del estado que lo identifica, y despues añadirá las transiciones, de manera que el agente pueda proseguir con du funcionamiento (ver la figura )
Los diferentes estados por los que puede pasar el agente son:
\begin{itemize}
	\item Atento: En el estado atento, el agente esta esperando a recibir un ACLMessage del tipo Request proveniente de la víctima. Cuando este mensaje se produce, el agente activa a los antagonistas del tipo Asesino a través de un ACLMessage del tipo INFORM con la ID ``Derrocar''.
	\item Rescate: En este estado el agente activará buscará de las paginas amarillas a aquellos agentes del tipo ``Matadragones'', que serán los aspirantes a héroes, buscando al menos uno disponible.
	\item Pedir Ayuda: Aquí se registrá un ACLMessage del tipo CFP, que se enviara a todos aquellos agentes encontrados en el estado anterior a través del MessageTemplate ``Solicitar Servicio''.
	\item Recibir Oferta: El agente recibirá la respuesta de cada aspirante a héroe a la petición del estado anterior a través de un ACLMessage. En este mensaje, ira incluido el precio por contratar sus servicios de cada aspirante, tras lo que el Allegado elegirá aquel con la mejor oferta, el precio mas bajo. Sino posee suficiente salario para pagar a un uevo aspirante, volverá a buscar aspirantes nuevos para ver si cumplen los requisitos de los que dispone.
	\item Aceptar Oferta: Al aspirante seleccionado finalmente en el anterior estado, el Allegado le mandará un ACLMessage del tipo ACCEPT PROPOSAL, en el que le proporciona los datos de la víctima y el secuestrador, de manera que el aspirante a héroe sea capaz de planificarse para rescatar a la víctima. Tras esto esperará una respuesta del aspirante en forma de ACLMessage. Si este mensaje es del tipo ``Inform'' implicará que el aspirante tuvo éxito en su empresa. Sino, significará que fallo, y el Allegado volverá a buscar nuevos aspirantes.
	\item Víctima Salvada: En este estado el Allegado enviará un ACLMessage de tipo Inform a la víctima, indicandola que ha sido rescatada, y eliminandola de la lista de víctimas a la que el Allegado esta ligado. Si esta lista de víctimas queda vacía o el rey se queda sin salario su ejecución terminará. Sino volvera al estado inicial.
\end{itemize}
...
Al iniciarse un nuevo aspirante, éste cargaba todos sus atributos, registraba el tipo "Matadragones" y se localizaba dentro del mapa.  El héroe registra dos comportamientos principales que son \texttt{OfrecerServicios()} y \texttt{AceptarOfertaRescate()}.
