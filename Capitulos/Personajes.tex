...
Estos atributos se cargaban a la hora de crear el personaje, y venían dictaminados por los diferentes archivos XML en los protagonistas, así como por su modificador por Raza, pues solo los protagonistas tenían estas características. Cada tipo de personaje tenía un atributo principal, que representaba su manera de afrontar la batalla, por ejemplo, un aspirante a héroe emplearía la fuerza para luchar mientras que una víctima usaría la destreza. La tabla final quedaría de la siguiente forma:
...
%-------------------------------------------------------------------
\subsection{Narrativa de los personajes}
Si se observa dentro del campo de la generación de narrativa computacional la mayoría de sistemas están ligados a un único tipo de historias, como se puede observar en el punto \ref{cap:EstadoDeLaCuestion:Narrativa}. Continuando el afán de crear un sistema altamente configurable, y también de poder dar la oportunidad a un escritor profesional crear una mejor narrativa para el sistema sin tener que entrar dentro de los apartados más técnicos del proyecto, se ha diseñado el apartado narrativo de la siguiente forma.

\section{Diseño de la narrativa}
Debido a que el sistema realizado esta enfocado en los personajes, serán estos mismos los encargados de almacenar toda la información lingüística que conocen. Para evitar que esta información esté ligada al código del sistema se ha decidido guardar esta información de forma externa y que el sistema se encargue de cargarla al iniciar el sistema. Con este diseño se consigue desligar el apartado narrativo del apartado técnico.

\subsubsection{Implementación de la narrativa}
La información narrativa del sistema se guarda dentro de un archivo XML. Actualmente la información se almacena en función del tipo de personaje y un atributo para trabajar en un entorno controlado, pero podría hacerse un archivo XML más extenso para que la lingüística de cada personaje fuese única, vinculando las frases con atributos más específicos.
...
Además de estar las frases vinculadas al tipo de personaje y un atributo, también se necesita vincular las frases a una acción determinada. Esto es debido a que las frases se invocaran dentro de los correspondientes \texttt{acciones o behaviurs} de cada personaje. También es posible almacenar más de una frase para cada tipo de acción para evitar que se repita la misma frase cada vez que se realiza la misma acción. La elección de la frase que se utiliza será aleatoria, por lo que a cuantas más frases pueda utilizar mayor variedad tendrá la narración de la historia. 

\subsubsection{Carga en el sistema}
Como ya se ha comentado la narrativa esta guardada en un archivo XML que tiene la siguiente composición:

\lstset{frameround=fttt}
\begin{lstlisting}[basicstyle=\scriptsize,frame=trBL,caption={Formato XML de la narrativa de los personajes},label=razasXML]
...
El archivo anterior tiene su propio \texttt{loarder} donde se carga todo el archivo de una vez por eficiencia y el \texttt{Mundo} será encargado de almacenar toda la narrativa cargada previamente. Cada personaje recibe la información lingüística correspondiente en el momento de su creación. 

\subsubsection{Ventajas del diseño}
La elección de este diseño ha sido para evitar el acoplamiento entre la parte técnica de la aplicación y la parte narrativa. El hacer esto da al sistema muchas variante ya que cambiando un único archivo de configuración se puede cambiar la forma narrativa de la historia en función del escritor o realizar traducciones en diferentes idiomas y hacer útil el sistema a nivel mundial.