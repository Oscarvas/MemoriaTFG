...
Las mejores historias siempre se caracterizan por un mapa detrás que las da soporte, que compone los cimientos del mundo que el autor quiere introducir. Este mapa dotará a la historia de individualidad y servirá para orientar al usuario, de manera que este sepa donde se produce la acción.
...
El anterior mapa del TFG %se podria referenciar de otra forma
...
Para ello creamos un archipiélago de islas, consultable en la figura \ref{fig:MiniMapa}, donde tres islas pequeñas coronaban una isla mayor, que a su vez se dividía en otras tres partes dividiendo así nuestro mundo en seis regiones donde cada cual tenía un numero diferente de localizaciones. También dotamos a cada una de estas regiones con un nombre para distinguirlas entre ellas.
...
Una vez correctamente digitalizado, lo introdujimos en la GUI, aportándonos un gran impacto visual y estando listo para consultar \ref{fig:MapaCompleto}.
...
Para que estas cargas se pudiesen realizar correctamente fueron necesarias la inclusión de dos nuevas clases, la clase Mapa y la clase Localización, que explicaremos en detalle mas adelante \ref{cap5:sec:ClaseMapaLocalizacion}. %aunque no se si es mejor poner que explicaremos al final del capitulo en vez de la ref
...
El problema que representaba esta implementación es que al necesitar el mapa de una configuración tan específica, el archivo XML debía crearse de una manera determinada, pues sino la aplicación no funcionaría de una manera correcta.
...
\subsection{Las Clases Mapa y Localización y como localizr personajes}
