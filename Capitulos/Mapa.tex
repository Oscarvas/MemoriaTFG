...
Para poder usarse de la manera correcta, primero se debía contar con un sistema cargador de datos (SCD) capaz de interpretar los datos del mapa. Sin embargo, los datos del mapa debían ser conocidos por el Agente \texttt{Mundo} y por tanto se integra este SCD dentro del propio agente dándole la funcionalidad de \texttt{CargarMapa()}.
...
De igual manera, la Clase \texttt{Localización} permitía al Agente \texttt{Mundo} conocer que personajes y objetos interactuan con ella así como con que localizaciones está conectada.

La Clase \texttt{Mapa} nos permite tener un conjunto de localizaciones fácilmente accesible por el Agente \texttt{Mundo} y acceder a localizaciones específicas de este.

De forma similar, para localizar a los personajes se creó la Interfaz \texttt{Vocabulario} junto al Enumerado \texttt{Mitología}, que permite ligar clases a tipos de localizaciones específicas para posteriormente elegir mediante una selección aleatoria dentro de la lista de localizaciones específicas de cada clase cual sería su localización inicial. 
