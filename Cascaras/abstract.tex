...
%sobre que hablamos
Automated Storytelling is a long path that began in the earlies 70's. The objective of this field is to provide a human quality, such as creativity, to an artificial intelligence in order to show this capability and therefore demonstrate that it is competent to tell a story. Though is true that this field has already been worked on and debated countless times, it is equally true that the solutions given do not cover the whole problem.
...
To fulfill this purpose, exists a specific field called Storyteller System through Intelligent Agents. These agents are software entities with an advanced artificial intelligence that allows them to perceive their environment ant interact with it, they are able to communicate with other agents as they try to success on the specific objectives that they will have throughout the story. These objectives will result on conflicts of interests between the different agents that will be solved through simulations, so when these simulations get finished they generate the story.
...
In order to be capable of doing that, is needed the platform JADE, which allows to interact with the agents, an extern planner and a simulator, capable of simulate what is needed.
...
The intention with this system is that the stories take place in different worlds, with many characters capable of acting in different ways in each simulation. These characters will have their own rules and features that will defined them as unique. They will also be able of interacting with objects and will have a more diverse system to tell their own phrases, greatly favoring diversity in the storytelling.
